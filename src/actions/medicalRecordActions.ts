...
export async function saveOrUpdateMedicalRecordAction(data: MedicalRecordDetailDTO) {
  ...
  try {
    ...
    return {
      error: error.message || "Hubo un problema al guardar la ficha médica.",
    };
  }
}
...
export async function getUserMedicalRecordAction() {
  ...
  try {
    ...
    return {
      error: error.message || "Hubo un problema al obtener tu ficha médica.",
    };
  }
}
...
export async function getPatientMedicalRecordAction(patientId: string | number) {
  ...
  try {
    ...
    return {
      error:
        error.message || "Hubo un problema al obtener la ficha médica del paciente.",
    };
  }
}
