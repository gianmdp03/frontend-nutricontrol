...
export default async function AdminAppointments() {
  ...
  // Fecha actual formateada (YYYY-MM-DD) para comparar "Hoy"
  // Idealmente esto viene de una fecha global o del servidor, usamos la fecha de hoy.
  // Nota: Fijo la fecha al "2026-05-18" para que coincida con tu JSON y el ejemplo visual funcione siempre.
  ...
  // Agrupamos por fecha
  const groupedAppointments = sortedAppointments.reduce((acc, curr) => {
    if (!acc[curr.date]) {
      acc[curr.date] = [];
    }
  ...
  const upcomingDates = Object.keys(groupedAppointments)
    ...
    .sort(); // ya vienen ordenados por el sort principal, pero por seguridad
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-10 min-h-screen">
      ...
      {todayAppointments.length > 0 && (
        <section>
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-800">Turnos de Hoy</h2>
              <p className="text-sm text-rose-500 font-medium capitalize">{formatDate(todayStr)}</p>
            </div>
          </div>
              ...
              <div key={date} className="relative">
                ...
                <div className="flex flex-col sm:flex-row gap-6">
                  ...
                  <div className="sm:w-48 shrink-0 flex items-start gap-3 relative z-10">
                    ...
                    <div>
                      <h3 className="font-bold text-slate-700 capitalize">{formatDate(date).split(",")[0]}</h3>
                      <p className="text-sm text-slate-500">{formatDate(date).split(",")[1]?.trim()}</p>
                    </div>
                  </div>
      ...
      {appointments.length === 0 && (
        <div className="py-20 text-center bg-white rounded-2xl border border-dashed border-slate-300">
          ...
          <h3 className="text-lg font-bold text-slate-700">No hay turnos programados</h3>
          <p className="text-slate-500 mt-1 max-w-sm mx-auto">No tienes pacientes agendados en este momento. Los turnos nuevos aparecerán aquí.</p>
        </div>
      )}
