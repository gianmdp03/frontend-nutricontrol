...
export default async function MyAppointments() {
  ...
  // Función para determinar si un turno está inactivo
  ...
  // Categorizar turnos
  const todayActiveAppointments = convertedAppointments.filter(
    (app) => app.date === todayStr && !isInactive(app.appointmentStatus)
  );

  const upcomingActiveAppointments = convertedAppointments.filter(
    (app) => app.date > todayStr && !isInactive(app.appointmentStatus)
  );

  const pastOrInactiveAppointments = convertedAppointments.filter(
    (app) => app.date < todayStr || isInactive(app.appointmentStatus)
  );

  // Ordenar próximos (más cercanos primero) y agrupar por fecha
  const sortedUpcoming = [...upcomingActiveAppointments].sort((a, b) => {
    if (a.date !== b.date) return a.date.localeCompare(b.date);
    return a.startTime.localeCompare(b.startTime);
  });

  const upcomingGrouped = sortedUpcoming.reduce(
    (acc, curr) => {
      if (!acc[curr.date]) {
        acc[curr.date] = [];
      }
  ...
  // Ordenar historial (más recientes primero) y agrupar por fecha
  const sortedPastOrInactive = [...pastOrInactiveAppointments].sort((a, b) => {
    if (a.date !== b.date) return b.date.localeCompare(a.date);
    return a.startTime.localeCompare(b.startTime);
  });

  const pastGrouped = sortedPastOrInactive.reduce(
    (acc, curr) => {
      if (!acc[curr.date]) {
        acc[curr.date] = [];
      }
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-10 min-h-screen">
      ...
      {todayActiveAppointments.length > 0 && (
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            {todayActiveAppointments.map((app: Appointment) => (
              ...
              <div key={date} className="relative">
                ...
                <div className="flex flex-col sm:flex-row gap-6">
                  ...
                  <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-4">
                    {upcomingGrouped[date].map((app: Appointment) => (
                      <PatientAppointmentCard
                        key={app.id}
                        appointment={app}
                        isToday={false}
      ...
      {/* SECCIÓN HISTORIAL (TURNOS PASADOS O INACTIVOS) */}
      {pastDates.length > 0 && (
        <section className="pt-6 border-t border-slate-200">
          <h2 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2">
            Historial de Turnos
            ...
            {pastDates.map((date) => (
              <div key={date} className="relative">
                ...
                <div className="flex flex-col sm:flex-row gap-6">
                  ...
                  {/* Tarjetas del día */}
                  <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-4">
                    {pastGrouped[date].map((app: Appointment) => (
                      <PatientAppointmentCard
                        key={app.id}
                        appointment={app}
