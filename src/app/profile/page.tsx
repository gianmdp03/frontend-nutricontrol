...
export default async function ProfilePage() {
  ...
  if (role === "ROLE_PATIENT") {
    ...
    try {
      certificates =
        await MedicalCertificateService.getPatientMedicalCertificates(token);
    } catch (e) {
      console.error(
        "Error loading patient medical certificates in ProfilePage:",
        e,
      );
    }
    ...
    try {
      nutritionalPlans =
        await NutritionalPlanService.getPatientNutritionalPlans(token);
    } catch (e) {
      console.error(
        "Error loading patient nutritional plans in ProfilePage:",
        e,
      );
    }
  }
            ...
            <div className="px-4 sm:px-8 pb-6 sm:pb-8 relative">
              <div className="relative flex flex-col sm:flex-row items-center sm:items-end justify-between -mt-12 mb-6 gap-4 sm:gap-0">
                ...
                <div>
                  <button
                    disabled
                    className="px-4 py-2 bg-slate-50 border border-slate-200 text-slate-400 text-sm font-medium rounded-lg cursor-not-allowed transition-colors w-full sm:w-auto"
                  >
              ...
              <>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 flex flex-col h-full">
                  ...
                  <div className="flex flex-col items-center justify-center flex-1 py-8 text-center bg-slate-50 rounded-xl border border-dashed border-slate-200">
                    ...
                    <p className="text-sm text-slate-600 font-medium">
                      Revisa tus turnos agendados o agenda uno nuevo.
                    </p>
                    <Link
                      href="/my-appointments"
                      className="mt-5 px-5 py-2.5 bg-rose-500 hover:bg-rose-600 text-white text-sm font-medium rounded-lg shadow-sm transition-all hover:shadow-md active:scale-95"
                    >
                      Ir a mis turnos
                    </Link>
                  </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 flex flex-col h-full">
                  ...
                  <div className="flex flex-col items-center justify-center flex-1 py-8 text-center bg-slate-50 rounded-xl border border-dashed border-slate-200">
                    ...
                    <p className="text-sm text-slate-600 font-medium">
                      Actualiza tu peso, altura, medicamentos e historial
                      médico.
                    </p>
                    <Link
                      href="/medical-record"
              ...
              <>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 flex flex-col h-full">
                  ...
                  <div className="flex flex-col items-center justify-center flex-1 py-8 text-center bg-slate-50 rounded-xl border border-dashed border-slate-200">
                    ...
                    <p className="text-sm text-slate-600 font-medium">
                      Accede a las herramientas de gestión del sistema.
                    </p>
                    <Link
                      href="/admin"
                      className="mt-5 px-5 py-2.5 bg-rose-500 hover:bg-rose-600 text-white text-sm font-medium rounded-lg shadow-sm transition-all hover:shadow-md active:scale-95"
