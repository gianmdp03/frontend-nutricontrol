...
import { medicalCertificateSchema, MedicalCertificateFormValues } from "@/schemas/MedicalCertificateSchema";
import { createMedicalCertificateAction } from "@/actions/medicalCertificateActions";
...
export default function MedicalCertificateFormModal({
  isOpen,
  onClose,
  patientId,
  patientName,
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<MedicalCertificateFormValues>({
  ...
  const onSubmit = (data: MedicalCertificateFormValues) => {
    ...
    startTransition(async () => {
      const res = await createMedicalCertificateAction(data);
      if (res.success) {
        setSuccessMessage("Certificado médico creado con éxito.");
        setTimeout(() => {
          onClose();
          setSuccessMessage(null);
        }, 1500);
      } else {
        setErrorMessage(res.error || "Hubo un error al registrar el certificado.");
      }
    });
  };
          ...
          >
            ...
            <div className="flex justify-between items-start border-b border-slate-100 pb-4 mb-6">
              <div>
                ...
                <h3 className="text-xl font-extrabold text-slate-800 mt-0.5">Crear Certificado Médico</h3>
              </div>
              <button
                type="button"
                onClick={onClose}
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              {/* Textarea Cuerpo */}
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 uppercase">Cuerpo / Texto del Certificado</label>
                <textarea
                  rows={6}
                  placeholder="Hace constar que el paciente goza de buena salud física / requiere reposo médico de..."
