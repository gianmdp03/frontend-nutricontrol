...
export default function PatientDocumentsView({
  ...
  const handleOpenPdf = async (id: number, type: "receta" | "certificado" | "nutritionalPlan") => {
    setLoadingDocId(id);
    try {
      const baseUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080/api";
      let endpoint = "";
      if (type === "receta") {
        endpoint = `${baseUrl}/prescriptions/user/${id}`;
      } else if (type === "certificado") {
        endpoint = `${baseUrl}/medical-certificates/user/${id}`;
      } else {
        endpoint = `${baseUrl}/nutritional-plans/user/${id}`;
      }
    ...
    <div className="bg-white rounded-3xl p-6 md:p-8 border border-slate-100 shadow-sm space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
        ...
        <div className="flex bg-slate-50 p-1 rounded-xl border border-slate-200/50 self-start sm:self-center">
          ...
          <button
            onClick={() => setActiveTab("nutritionalPlans")}
            className={`px-4 py-2 rounded-lg text-xs font-bold transition-all cursor-pointer ${
              activeTab === "nutritionalPlans"
                ? "bg-white text-rose-500 shadow-xs"
                : "text-slate-500 hover:text-slate-800"
            }`}
          >
            Planes Nutricionales ({nutritionalPlans.length})
          </button>
        </div>
      </div>
          ...
          nutritionalPlans.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {nutritionalPlans.map((plan) => (
                ...
                >
                  <div className="space-y-2">
                    <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest block">Plan Nutricional</span>
                    <h3 className="font-extrabold text-slate-800 text-sm line-clamp-2">
                      {plan.textareaTexto}
                    </h3>
                    <p className="text-xs text-slate-400 font-semibold">
                      Dra. {plan.adminName || "Especialista"} • {plan.specialty || "Nutrición"}
                    </p>
                  </div>

                  <div className="pt-4 mt-4 border-t border-slate-200/50 flex items-center justify-between text-xs">
                    <span className="text-slate-400 font-medium">{formatDate(plan.date || plan.dateTime)}</span>
                    <button
                      onClick={() => handleOpenPdf(plan.id, "nutritionalPlan")}
                      disabled={loadingDocId === plan.id}
                      className="px-4 py-2 bg-rose-500 hover:bg-rose-600 text-white font-bold rounded-lg shadow-xs hover:shadow-md active:scale-95 transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
                    >
                      {loadingDocId === plan.id ? (
                        <>
                          <div className="w-3.5 h-3.5 border-2 border-white/20 border-t-white rounded-full animate-spin"></div>
                          <span>Cargando...</span>
                        </>
            ...
            <div className="py-12 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200 p-6">
              ...
              <h4 className="font-bold text-slate-700 text-sm">Sin planes nutricionales emitidos</h4>
              <p className="text-xs text-slate-500 mt-1 max-w-[280px] mx-auto leading-relaxed">
                Tus planes nutricionales oficiales en formato PDF se listarán aquí en cuanto sean emitidos.
              </p>
            </div>
          )
